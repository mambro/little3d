...
const char * meshfprojective = GLSL330(
	out vec4 color;
...
const char * meshvdecal = GLSL330(
	...
	out vec3 DataIn_lightDir;
	out vec3 DataIn_eye;
	out vec3 DataIn_fragpos;
	out vec4 DataIn_fragposprj;

	void main(){
		vec4 pos = m_VM * vertexPosition_modelspace;
		DataIn_fragpos = m_M * vertexPosition_modelspace;
    	DataIn_normal = normalize(m_N * vertexNormal_modelspace);
    	DataIn_lightDir = vec3(l_pos - pos.xyz);
    	DataIn_eye = vec3(-pos);
    	DataIn_fragposprj = m_DM * vec4(DataIn_fragpos,1.0);

	    gl_Position =  m_P*pos;
	}
);

const char * meshfdecal = GLSL330(
	out vec4 color;
	uniform vec4 diffuse;
	uniform vec4 ambient;
	uniform vec4 specular;
	...
	uniform sampler2D projective;
	in vec4 DataIn_fragposprj;

	void main()
	{
		vec3 projCoords = DataIn_fragposprj.xyz / DataIn_fragposprj.w;
		vec4 spec = vec4(0.0);
